...
class vec 
   ...
   @Override
   public String toString() {return "Vec: (" + x + ", " + y + ", " + z + ")";}
...
class pt 
   ...
   @Override
   public String toString() {return "pt: (" + x + ", " + y + ", " + z + ")";}
   }
...
pt R(pt Q, float a, pt C) {float dx=Q.x-C.x, dy=Q.y-C.y, c=cos(a), s=sin(a); return P(C.x+c*dx-s*dy, C.y+s*dx+c*dy, Q.z); };  // Q rotated by angle a around point C
...
void label(pt P, String S) {text(S, ToScreen(P).x, ToScreen(P).y); }                                  // writes string S at P+V